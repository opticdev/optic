...
export const allowedMethods = ['GET', 'HEAD', 'POST', 'PUT', 'DELETE', 'CONNECT', 'OPTIONS', 'TRACE', 'PATCH'];

export function parseRule(userInput: string): IIgnoreRunnablePredicate | undefined {
  ...
  if (methodArray.length === 0) {
    ...
    methods = methodArray.map(i => i.toUpperCase()).filter(i => allowedMethods.includes(i));
  }

  const regex = pathToRegexp(pathInput);
  const shouldIgnore = (method: string, url: string) => methods.includes(method) && regex.exec(url) !== null;
