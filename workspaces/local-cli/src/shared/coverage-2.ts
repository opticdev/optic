...
// o.O
const createDataClass = <P extends Array<any>>(
  func: (...args: P) => void
): ((...args: P) => string) => {
  return (...args: P) =>
    `${func.name}-${args.map((arg) => JSON.stringify(arg)).join('-')}`;
};
...
const TotalForPathAndMethod = createDataClass(function TotalForPathAndMethod(
  pathId: String,
  httpMethod: String
) {});
const TotalForPathAndMethodAndStatusCode = createDataClass(
  function TotalForPathAndMethodAndStatusCode(
    pathId: String,
    httpMethod: String,
...
const TotalForPathAndMethodWithoutBody = createDataClass(
  function TotalForPathAndMethodWithoutBody(
    pathId: String,
    httpMethod: String
  ) {}
);
const TotalForPathAndMethodAndContentType = createDataClass(
  function TotalForPathAndMethodAndContentType(
    pathId: String,
    httpMethod: String,
...
const TotalForPathAndMethodAndStatusCodeWithoutBody = createDataClass(
  function TotalForPathAndMethodAndStatusCodeWithoutBody(
    pathId: String,
    httpMethod: String,
    httpStatusCode: number
  ) {}
...
const TotalForPathAndMethodAndStatusCodeAndContentType = createDataClass(
  function TotalForPathAndMethodAndStatusCodeAndContentType(
    pathId: String,
    httpMethod: String,
    httpStatusCode: number,
...
const TotalForRequest = createDataClass(function TotalForRequest(
  requestId: string
) {});
const TotalForResponse = createDataClass(function TotalForResponse(
  responseId: string
) {});
...
async function computeCoverage(paths: IPathMapping, captureId: string) {
  ...
  const coverages = (async function* (interactionItems) {
    for await (let item of interactionItems) {
      ...
      const diff_str = DiffEngine.diff_interaction(
        JSON.stringify(item.interaction),
        spec
      );
...
function interactionToCoverage(
  spec: DiffEngine.WasmSpecProjection,
  ...
  const path_id = DiffEngine.spec_resolve_path_id(
    spec,
  ...
  if (path_id) {
    ...
    const request_data = JSON.parse(
      DiffEngine.spec_resolve_request(
        spec,
        path_id,
    ...
    if (!request_data) {
      ...
      recordEvent(
        TotalForPathAndMethod(path_id, interaction.request.method),
        coverage_map
      );
      ...
      if (request.body) {
        recordEvent(
          TotalForPathAndMethodAndContentType(
            path_id,
            interaction.request.method,
            request.body.http_content_type
          ),
          coverage_map
        );
      } else {
        recordEvent(
          TotalForPathAndMethodWithoutBody(path_id, interaction.request.method),
          coverage_map
        );
      }

      const response_data = JSON.parse(
        DiffEngine.spec_resolve_response(
          spec,
          interaction.request.method,
        ...
        if (response.body) {
          recordEvent(
            TotalForPathAndMethodAndStatusCodeAndContentType(
              ...
              interaction.request.method,
              interaction.response.statusCode,
              response.body.http_content_type
            ),
            coverage_map
          );
        } else {
