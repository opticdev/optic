...
class SerializeProjectGraphSpec extends AkkaTestFixture("DiffSyncGraphSpec") with SyncFixture with GearUtils with TestPackageProviders {
  ...
  it("can use snapshot functionality to extract all named objects from project") {
    ...
    assert(result.toJson == Json.parse("""[{"name":"Welcome To","expandedValue":{"value":"welcome to"},"schema":"optic:synctest@0.1.0/source-schema"},{"name":"Hello Model","expandedValue":{"value":"hello"},"schema":"optic:synctest@0.1.0/source-schema"},{"name":"Good Morning","expandedValue":{"value":"good morning"},"schema":"optic:synctest@0.1.0/source-schema"}]"""))
  }
