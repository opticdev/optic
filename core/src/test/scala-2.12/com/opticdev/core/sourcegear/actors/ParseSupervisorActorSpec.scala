...
class ParseSupervisorActorSpec extends AkkaTestFixture("ParseSupervisorActorTest") {
  ...
  describe("context lookup") {
    ...
    it("for file in cache") {
      ...
      f.actorCluster.parserSupervisorRef ! AddToCache(file, Graph(), SourceParserManager.installedParsers.head, "Contents", None, FileTokenRegistry(), FileImportsRegistry())
      f.actorCluster.parserSupervisorRef ! GetContext(file)(sourceGear, project)
      expectMsg(Option(SGContext(sourceGear.fileAccumulator, Graph(), SourceParserManager.installedParsers.head, "Contents", null, file, null, null, null)))
    }
  ...
  describe("caches") {
    val f = fixture
    val dummyRecord = CacheRecord(Graph(), null, "contents", None, FileTokenRegistry(), FileImportsRegistry())
    ...
    it("can add records") {
      f.actorCluster.parserSupervisorRef ! ClearCache
      f.actorCluster.parserSupervisorRef ! AddToCache(file, Graph(), null, "contents", None, FileTokenRegistry(), FileImportsRegistry())
