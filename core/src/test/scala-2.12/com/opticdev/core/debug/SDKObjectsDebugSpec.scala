...
class SDKObjectsDebugSpec extends TestBase with TestPackageProviders {
  ...
  describe("Lens Debug Information") {
    ...
    it("Is collected during compile") {
      ...
      assert(lensDebug.gearHash.contains("1ba052b9"))
    }

    it("Can be turned into JSON") {
      val lensDebug = LensDebug.run(importLens.lens, importLens.packageContext)
      assert(lensDebug.toJson == Json.parse("{\"isSuccess\":true,\"snippet\":{\"language\":\"es7\",\"block\":\"let definedAs = require('pathTo')\"},\"componentsInfo\":{\"isSuccess\":true,\"found\":[{\"component\":{\"propertyPath\":\"definedAs\",\"finder\":\"Entire definedAs \"},\"range\":{\"start\":4,\"end\":13}},{\"component\":{\"propertyPath\":\"pathTo\",\"finder\":\"Containing pathTo \"},\"range\":{\"start\":24,\"end\":32}}],\"notFound\":[]},\"containersInfo\":{\"isSuccess\":true,\"found\":[],\"notFound\":[]},\"variables\":[],\"gearHash\":\"1ba052b9\",\"sdkType\":\"lens\"}"))
    }
