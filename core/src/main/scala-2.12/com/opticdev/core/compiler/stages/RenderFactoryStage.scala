...
class RenderFactoryStage(snippetStageOutput: SnippetStageOutput, modelsParseGear: ParseGear)(implicit lens: Lens) extends CompilerStage[GeneratorFactoryOutput] {
  override def run: GeneratorFactoryOutput = {
    GeneratorFactoryOutput(new RenderGear(
