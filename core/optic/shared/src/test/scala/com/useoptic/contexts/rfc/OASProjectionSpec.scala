...
class OASProjectionSpec extends FunSpec with JsonFileFixture {
  ...
  it("works for basic todo app") {
    ...
    val oas = new OASProjection(queries, rfcService, "id", "ToDo")
  ...
  it("works for basic dry todo app") {
    ...
    val oas = new OASProjection(queries, rfcService, "id", "Unnamed API")
  ...
  describe("Json Schema") {
    ...
    it("works for todo shape") {
      val (queries, rfcService, rfcState) = fixture("todo")
      val todoSchema = new JsonSchemaProjection(queries,"jghsd0_1").asJsonSchema(expand = false)
