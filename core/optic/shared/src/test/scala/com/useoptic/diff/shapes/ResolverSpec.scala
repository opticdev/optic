...
class ResolverSpec extends FunSpec {
  describe("resolving trails") {
    describe("given a spec with a request body that is an object") {
      ...
      val Resolvers = new UncachedSpecResolvers(rfcState)
  ...
  describe("resolving json from trails and interactions") {
    describe("primitives") {
      it("should resolve the value") {
        val resolved = JsonResolver.tryResolveJsonTrail(JsonTrail(Seq()), JsonLikeFrom.json(json""""string""""))
        assert(resolved.get.asJson == json""""string"""")
      }
    }
    describe("object") {
      it("should resolve the value") {
        val resolved = JsonResolver.tryResolveJsonTrail(JsonTrail(Seq(JsonObject(), JsonObjectKey("k"))), JsonLikeFrom.json(json"""{"k":1}"""))
        assert(resolved.get.asJson == json"""1""")
      }
      it("should not resolve a value") {
        val resolved = JsonResolver.tryResolveJsonTrail(JsonTrail(Seq(JsonObject(), JsonObjectKey("notk"))), JsonLikeFrom.json(json"""{"k":1}"""))
        assert(resolved.isEmpty)
      }
    }
    describe("array") {
      it("should resolve the value") {
        val resolved = JsonResolver.tryResolveJsonTrail(JsonTrail(Seq(JsonArray(), JsonArrayItem(0))), JsonLikeFrom.json(json"""[1]"""))
        assert(resolved.get.asJson == json"""1""")
      }
      it("should not resolve a value") {
        val resolved = JsonResolver.tryResolveJsonTrail(JsonTrail(Seq(JsonArray(), JsonArrayItem(1))), JsonLikeFrom.json(json"""[1]"""))
        assert(resolved.isEmpty)
      }
    }
    describe("nested") {
      ...
      it("should resolve .k[0].x[1] => 'b'") {
        ...
        val resolved = JsonResolver.tryResolveJsonTrail(trail, JsonLikeFrom.json(nested))
