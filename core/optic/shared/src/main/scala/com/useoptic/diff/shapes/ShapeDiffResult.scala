...
sealed trait ShapeDiffResult extends DiffResult {
  ...
  def withNormalizedJsonTrail: ShapeDiffResult = {
    this match {
      case d: UnspecifiedShape => d.copy(jsonTrail = jsonTrail.withNormalizedJsonTrail)
      case d: UnmatchedShape => d.copy(jsonTrail = jsonTrail.withNormalizedJsonTrail)
    }
  }
}
