    ...
    val newRegions = interactionsGroupedByDiffs.collect {
      case (diff: UnmatchedRequestBodyContentType, interactions) => {
        ...
        val previewRender = (interaction: HttpInteraction) => BodyUtilities.parseBody(interaction.request.body).map { body =>
          DiffPreviewer.previewDiff(Some(body), rfcState, None, Set.empty, ungroupedDiffs)
        }
    ...
    val requestShapeRegions = interactionsGroupedByDiffs.filter {
      ...
      .flatMap { case (contentType, diffMap) => diffMap.map(i => {
        ...
        val responseRender = (interaction: HttpInteraction, withRfcState: Option[RfcState]) => Try {
          ...
          DiffPreviewer.previewDiff(BodyUtilities.parseBody(interaction.response.body), innerRfcState, Some(responseShapeID), Set.empty, ungroupedDiffs)
        }.toOption
    ...
    val responseShapeRegions = interactionsGroupedByDiffs.filter {
      ...
      .flatMap { case (contentType, diffMap) => diffMap.map(i => {
        ...
        val requestRender = (interaction: HttpInteraction, withRfcState: Option[RfcState]) => Try {
          ...
          DiffPreviewer.previewDiff(BodyUtilities.parseBody(interaction.request.body), innerRfcState, Some(requestBodyShapeId), Set.empty, ungroupedDiffs)
        }.toOption
