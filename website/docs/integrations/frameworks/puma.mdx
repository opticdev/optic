---
title: Using Puma with Optic
sidebar_label: Puma
slug: /puma
---

Optic is easy to use with Puma, no special library or large code changes required.

## `api start` Command

Let's say we have a simple Puma server that we usually develop with on localhost:3005.

### Optic needs to know how to start our API and control the port it starts on

Our optic.yml file would include our start command (such as `puma -p $PORT`). The start command in this case also sets the port to which the application binds. Optic injects a `$PORT` environment variable for our application to listen on when `api start` is run. Puma accepts the `-p` option which sets the port to which the application will bind. There should not be any code changes required.
