...
- **Recommended for local development** [Have Optic start your API for you and intercept traffic](/docs/get-started/config/basic-task)
- [Set up Optic as a proxy for your environment](/docs/get-started/config/proxy)

You may also explore our docs to learn more about [running your tests with Optic](/docs/get-started/testing), [installing the GitBot](/docs/apiops/pull-requests), and [sharing Optic with your team](docs/using/share-with-team).
...
<Tabs
  ...
  values={[
    {label: 'Intercept Environment (Recommended)', value: 'intercept'},
    {label: 'Basic Task', value: 'basic'},
...
**When the intercept mode may be counter-indicated:** If your application runs locally and you'd like to test against it, we'd recommend using a [basic task](/docs/get-started/config/basic-task) as it helps manage your project lifecycle and sets up the proxy to be friendly to local tests and workflows.
...
**When would I use a basic task?** Basic tasks are ideal when running a project locally, as it manages not only the Optic proxy but the application lifecycle. It allows for Optic to step in on the port your project usually starts on, making integration with existing tests and workflows seamless. 

**When a basic task may be counter-indicated:** If the project lifecycle is not easily managed with a single command, or the project is located on a remote system (such as behind an Amazon API Gateway), we'd recommend using an [intercept environment](/docs/get-started/config/intercept) as it is more feature-rich. 
...
**Instead of a proxy task:** When running a project locally, we'd recommend managing the application lifecycle with a [basic task](/docs/get-started/config/basic-task).
