...
## Transparent Proxy task mode

For most local documentation, we recommend starting your API application directly with an [Optic task](/docs/), also known as Command Wrapper mode. This allows Optic to start your project and a new capture session to organize observed requests. Sometimes, this isn't feasible - such as when your API is always deployed in a Serverless architecture and cannot be run locally. In this case, the Optic Transparent Proxy mode will let you configure your downstream and upstream URLs.

### Setting up the Local CLI in Transparent Proxy mode

All configuration items for local capture live in the `optic.yml` file. Create a new task to use the Transparent Proxy mode:
...
- The `command` field is for process wrapping, and isn't necessary for the transparent proxy configuration. Leave that out.
...
### Starting a new Capture Session in Transparent Poxy task mode

When Optic is started in Transparent Proxy task mode, it is not watching the application lifecycle and does not currently capture rebuild events of the application under observation. To start fresh with a new capture session, stop the current Optic proxy and restart with `api run start-proxy`. This will create a new capture session in the Optic dashboard and allow you to observe traffic sent to the most recent build of the application under observation.
