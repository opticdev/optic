    ...
    return (
      <Sheet>
        <div className={classes.root}>
          ...
          <Typography variant="subtitle2" style={{paddingTop: 11, paddingBottom: 11}}>Optic observed a new URL. Before
            Optic can document the requests you need to add a matching path to your API specification.</Typography>
        ...
        <div>
          {matchingUrls.size ? <Typography variant="subtitle1" style={{paddingTop: 11, paddingBottom: 11}}>Other Matching URLs:</Typography> : null}
          <List style={{marginTop: -15}}>
            {urls.filter(i => matchingUrls.has(i)).map(url => {
              return (
                <ListItem><ListItemText primary={url} primaryTypographyProps={{fontSize: 12}}/></ListItem>
              );
            })}
          </List>
        </div>
