...
trait ConnectionManager[A <: Connection] {
  ...
  def apply(slug: String)(implicit actorSystem: ActorSystem) : A

  protected var connections: Map[String, A] = Map()

  def listConnections = connections

  def findOrCreate(slug: String)(implicit actorSystem: ActorSystem): A = {
    connections.getOrElse(slug, createEditorConnection(slug))
  }

  private def createEditorConnection(slug: String)(implicit actorSystem: ActorSystem): A = {
    val connection = apply(slug)
    connections += slug -> connection
    connection
  }

  def websocketChatFlow(connection: A)(implicit executionContext: ExecutionContext): Flow[Message, Message, Any] =
    Flow[Message]
      .collect {
