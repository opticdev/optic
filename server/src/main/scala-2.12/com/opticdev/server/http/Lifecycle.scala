...
object Lifecycle extends App {
  ...
  val jar = this.getClass.getClassLoader.getResource("es7_2.12-0.1.4.jar").getFile
  val parserBase = Try {
    ParserStorage.writeToStorage(File(jar))
    SourceParserManager.installParser(jar).get
  }.getOrElse {
    val withoutFileExtension = jar.substring(5)
    val pathAsString = (File(withoutFileExtension).parent.parent / "es7_2.12-0.1.4.jar").pathAsString
