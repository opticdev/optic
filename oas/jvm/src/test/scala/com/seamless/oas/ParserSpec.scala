...
class ParserSpec extends ResolverTestFixture("2") {
  ...
  it("works on a version 2 when valid") {
    val mattermost = Parser.parseOAS(pathToContents("jvm/src/test/resources/mattermost-2.json"))
    println(mattermost)
  }

  it("works on a version 3 when valid") {
    val bbc = Parser.parseOAS(pathToContents("jvm/src/test/resources/bbc-3.json"))
    println(bbc)
  }
