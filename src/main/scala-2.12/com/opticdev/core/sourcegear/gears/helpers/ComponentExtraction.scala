        ...
        case c: CodeComponent => {
          ...
          c.codeType match {
            case Literal=> {
              ...
              ModelField(component.propertyPath, valueOption.get, Node(node))
            }
            case Token=> {
              ModelField(component.propertyPath, JsString(fileContents.substring(node.range._1, node.range._2)), Node(node))
            }
          }
