...
import Fixture.{FileUtils, TestBase}
  ...
  describe("Project file") {
    ...
    describe("instantiates from a YAML file") {
      it("should work when valid") {
        new ProjectFile(File(getCurrentDirectory + "/src/test/resources/tmp/example_project_files/project.optic"))
      }
      it("should throw if the file does not exist") {
        assertThrows[InvalidProjectFileException] {
          new ProjectFile(File(getCurrentDirectory + "/src/test/resources/tmp/example_project_files/notReal.optic"), false)
        }
      }
      it("should throw when file is invalid YAML") {
        assertThrows[InvalidProjectFileException] {
          new ProjectFile(File(getCurrentDirectory + "/src/test/resources/tmp/example_project_files/invalidFile.optic"), false)
        }
      }
    }
    ...
    def fixture = new {
      val defined = new ProjectFile(File(getCurrentDirectory + "/src/test/resources/tmp/example_project_files/project.optic"))
      val empty = new ProjectFile(File(getCurrentDirectory + "/src/test/resources/tmp/example_project_files/empty.optic"))
    }
