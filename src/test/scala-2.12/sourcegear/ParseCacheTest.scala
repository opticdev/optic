...
import com.opticdev.core.sourcegear.{CacheRecord, ParseCache, SGConstants}
  ...
  describe("Parse Cache") {
    ...
    describe("additions") {
      ...
      it("bump file to the top if already in buffer") {
        ...
        parseCache.add(file1, dummyRecord)
        parseCache.add(FileNode("/src/test/resources/tmp/test_project/2", "ABC"), dummyRecord)
        parseCache.add(file1, dummyRecord)
      ...
      it("does not duplicate file when hash is different, just replaces/bumps") {
        parseCache.clear
        parseCache.add(FileNode("/src/test/resources/tmp/test_project/2", "ABC"), dummyRecord)
        parseCache.add(FileNode("/src/test/resources/tmp/test_project/2", "BCD"), dummyRecord)
      ...
      it("will remove oldest cache when exceeds size limit") {
        ...
        parseCache.add(file1, dummyRecord)
        parseCache.add(FileNode("/src/test/resources/tmp/test_project/2", "ABC"), dummyRecord)
        parseCache.add(FileNode("/src/test/resources/tmp/test_project/3", "ABC"), dummyRecord)
        parseCache.add(FileNode("/src/test/resources/tmp/test_project/4", "ABC"), dummyRecord)
        parseCache.add(FileNode("/src/test/resources/tmp/test_project/5", "ABC"), dummyRecord)
