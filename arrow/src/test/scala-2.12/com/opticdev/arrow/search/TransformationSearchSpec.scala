...
class TransformationSearchSpec extends FunSpec {
  ...
  it("can convert transformation result to JSON ") {
    ...
    assert(results.head.asJson.toString() == "{\"name\":\"Model -> Route\",\"projectName\":\"test\",\"packageId\":\"optic:test-transform@latest\",\"inputSchema\":\"optic:test@latest/model\",\"outputSchema\":\"optic:test@latest/route\",\"changes\":[{\"transformationChanges\":{\"transformation\":{\"name\":\"Model -> Route\",\"packageId\":\"optic:test-transform@latest\",\"inputSchema\":\"optic:test@latest/model\",\"outputSchema\":\"optic:test@latest/route\",\"code\":\"\"},\"target\":\"optic:test@latest/route\",\"_type\":\"com.opticdev.arrow.graph.KnowledgeGraphImplicits.DirectTransformation\"},\"gearOptions\":[],\"locationOptions\":[]}]}")
