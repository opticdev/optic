...
object GearSearch {
  def search(query: String, context: ArrowContextBase)(implicit sourcegear: SourceGear, project: OpticProject) : Vector[Result] =
    search(query, context, sourcegear.gearSet.listGears)

  def search(query: String, context: ArrowContextBase, gears: Set[Gear])(implicit sourcegear: SourceGear, project: OpticProject) : Vector[Result] =
    gears
      .map(i=> {
